...
MARCUP.add(find_button, check_button, report_button, info_button, education_button)
...
def get_education_information(message):
    bot.send_message(message.chat.id, "За хранение экстремистских материалов в целях массового распространения "
                                      ...
                                      "суток (ст. 20.29 КоАП РФ).\nПод экстремистскими материалами понимаются "
    ...
    bot.send_message(message.chat.id, "Если Вы обнаружили в "
...
def help_handler(message):
    bot.send_message(message.chat.id,
                     "Список команд:\n1)Получить информацию о боте - /info Проверка материала на наличие в реестре запрещённых материалов - /find [name]"
                     "\n2)Проверка списка материалов в реестре на наличие запрещённой литературы - /check"
                     "\n3) Отправить жалобу на материал - /report "
                     "\n4) ", reply_markup=MARCUP)
